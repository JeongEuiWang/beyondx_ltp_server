...
FILE_PATH = PROJECT_ROOT / 'app' / 'db' /'migrations' /'resource' / 'Texas' / 'Texas_location.csv'
...
def insert_region() -> None:
    ...
    op.bulk_insert(
        rate_region_table,
        [
            {"id": 1, "name": "Texas", "description": "Dallas-Fort Worth Region", "created_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S'), "updated_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')},
        ]
    )
...
def insert_area() -> None:
    ...
    op.bulk_insert(
        rate_area_table,
        [
            {"id": 1, "region_id": 1, "name": "A", "min_order_amount": 25, "max_load": 225, "created_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S'), "updated_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')},
            {"id": 2, "region_id": 1, "name": "B", "min_order_amount": 30, "max_load": 250, "created_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S'), "updated_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')},
            {"id": 3, "region_id": 1, "name": "C", "min_order_amount": 35, "max_load": 275, "created_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S'), "updated_at": datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')},
        ]
    )
    pass
...
def extract_location(file_path: Path) -> list[dict]:
    with open(file_path, mode='r', encoding='utf-8-sig') as file:
        ...
        for row in reader:
            ...
            temp_row_dict['created_at'] = datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')
            temp_row_dict['updated_at'] = datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S')
