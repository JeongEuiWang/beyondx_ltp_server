...
from app.schema.cost import BaseCost
from app.model._enum import LocationTypeEnum, ShipmentTypeEnum
from app.schema.quote import UpdateQuoteRequest, QuoteLocationRequest, QuoteCargoRequest, QuoteLocationAccessorialRequest
...
class TestQuoteAPI:
    ...
    async def test_create_quote_max_load_exception(self, client):
        ...
        with patch("app.service.cost.CostService.calculate_base_cost") as mock_calculate_base_cost:
            # 모의 응답 설정
            mock_calculate_base_cost.return_value = BaseCost(
            ...
            assert "최대 금액을 초과했습니다" in response.json()["detail"]
